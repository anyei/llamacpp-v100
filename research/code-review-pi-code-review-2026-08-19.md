...
- [x] 2 PEARL decode-error throw before join / reload UAF — fixed 2026-08-23 (join moved to immediately after target decode, before ANY result handling, so prompt_clear/throw/retry paths never run under a live worker; belt-and-suspenders joins in abort_all_slots() and destroy() cover the catch-handler and reload teardown; gate: build clean, on/off byte-identical d4aafddd, 12 adoptions, np2 concurrent leg alive 12x200)
...
- [x] 5 deferred cache eviction races new manifest — fixed 2026-08-23 (g_rpc_cache_evict_mutex serializes delete loop vs manifest dir scan + per-iteration conn recheck aborts eviction the moment a coordinator connects; gate: byte sha c80261ff on no-cache/cached/reconnect legs, forced-race leg with 50k padded entries hit 'eviction aborted' live on 2/3 workers with 0 stale-manifest and RELOAD-OK)
...
- [x] 11 adopted PEARL rounds skip update_tgt — fixed 2026-08-24 (adoption path captures the target checkpoint with the same use_ckpt_tgt condition as the drafting iterate, right after update_pos, so a partial rejection of an adopted draft restores this round's blob; PART targets unaffected (condition false). Gate: gate1-pearl regression d4aafddd off/on, 12 adoptions, np2 12/12 alive; the FULL/RS-target corruption shape has no same-vocab PART-drafter vehicle in /work - code-trace verified, real-serve check when a SWA+drafter pair exists)
- [x] 12 ctx_hold_begin teardown with guards held — fixed 2026-08-24 (CONFIRMED via repro: a guard-holder outlasting the 15s window let ctx_hold_begin proceed to destroy() the model under it. ctx_hold_begin now returns bool; on timeout with guards still held the reload drops the hold, re-arms rpc_reload_pending, and returns WITHOUT freeing - the next update_slots retries once the slow reader drains. Repro (LLAMA_TEST_PRETASK_SLEEP_MS one-shot + --rpc-reload + LLAMA_RPC_NO_SURGICAL + worker kill): pre-fix = teardown proceeded + destroy while guard held; post-fix = 2 deferrals while held, reload completes on the 3rd attempt after drain, 0 crashes. Normal reload (no slow reader) = 1 attempt no defer, served 200. Byte gate c80261ff. Repro instrumentation was temp, NOT committed)
- [x] 13 process_rows branch-heal misindexes MTP/EAGLE3 — fixed `c2f43e1f5` (2026-08-21, #137 step 0)
- [x] 14 spec-tree row demand vs batch capacity — fixed 2026-08-24 (per-round branch budget: n_batch minus every generating slot's main anchor+draft rows, arming draws from it and disarms when short, so main rows can never hit the add_ok assert; spec_tree_i_batch rows joined the post_decode sub-batch view guard like spec_i_batch. Gate: tree-off b8272afb exact; tree np1 long leg 64+ armed rounds clean; crush leg -b 14 np2 draft4 = the pre-fix assert shape, 6/6 responses, health 200, 0 aborts; 200-token 500 = content-format validator on the garbage vehicle, reproduced tree-off)
...
- [x] 18 RPC graph-uid cross-process collisions — fixed 2026-08-24 (uid high half = per-process random salt (splitmix64 of pid^time^addr, CAS-once init) so two client processes on one worker can no longer claim each other's cached graphs; server uid cache eviction FIFO -> LRU (refresh on recompute hit and re-upload) so active uids survive other clients' inserts; :391 comment corrected. Gates: gate6 byte identity c80261ff 6/6 x3 legs; two-coordinator concurrent + restart + idle-wake shapes all stable, 0 owner-conflicts/misses/drops. Pre-fix controls on all 3 shapes could NOT provoke counter alignment (timing-fragile lottery - the exact thing the salt removes); mechanism confirmed by review. Residual: recompute-miss still drops the conn past ~8 active contexts/device - the banked miss->reupload protocol hardening stays open)
- [x] 19 RPC cache cap unenforced while conns live — fixed 2026-08-24 (live-session registry: conns register their announced model, un-announced conns count as flat-dir users; eviction runs with conns live but skips every live folder (+flat while legacy conns exist), keeps the #5 abort-on-new-conn, time-boxes mid-serve delete loops at 500ms, rate-limits per-save scans to 10s - first attempt without the rate limit crawled loads n^2, caught by gate5 legs going 503-ERR. Live folders hard-protected regardless of enforcing thread; last-closer soft preference kept for idle ordering. Gates: gate5 all legs c80261ff + RELOAD-OK + 0 stale manifests + yield-aborts firing; gate19 cold folder 4000->0 mid-serve, live model intact, canonical bytes)
...
- [x] 29 SESSION_MODEL once per socket (PLAUSIBLE, latent) — fixed 2026-08-24 (manifest_ensure re-announces + re-fetches when g_rpc_session_model changes on a live socket, clearing the old scope's manifest up front so a failed refetch degrades to streaming; latch is now per-scope not per-socket. CONFIRMED LATENT/INERT: gate29 (target + remote dspark drafter over one worker) shows a SINGLE announce - no current caller re-announces (drafters never set a session id), so the rescope branch does not execute in any present flow; gate6 byte-identity c80261ff proves the non-rescope path unchanged. Defensive hardening for a future multi-model-per-socket flow; single-scope manifest scoping proven working (257 entries, correct folder, 0 stale). Note: gate29's decode -3 is the fragile trunc-V4-on-single-RPC-CPU placement, unrelated - 0 uid/manifest/abort events)
