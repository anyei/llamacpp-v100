...
ggml_tensor * llm_build_delta_net_base::build_conv_state(
    ...
    if (cparams.n_rs_seq == 0) {
        ...
        // the last (n_rs_seq + 1) tokens of a sequence must be inside the same ubatch; this is
        // guaranteed by split_equal(..., full_seqs = true), which never splits a sequence across ubatches
