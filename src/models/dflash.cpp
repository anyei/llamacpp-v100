...
void llama_model_dflash::load_arch_tensors(llama_model_loader &) {
    ...
    // DFlash2 = DFlash + dynamic conv mixing and a learned candidate selector
    ...
    if (selector_meta) {
        ...
        if ((uint32_t) n_embd < hparams.dflash_selector_top_k * (hparams.dflash_selector_top_k + 1)) {
...
template <>
llama_model_dflash::graph<true>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context_dsv4_mla(params), model(model) {
...
// DFlash2: block-causal grouped conv with input-dependent tap coefficients
// (dynamic, from conv_proj) plus a static per-tap base; side 0 = pre, 1 = post
...
template <>
llama_model_dflash::graph<false>::graph(const llama_model & model, const llm_graph_params & params) : llm_graph_context_dsv4_mla(params), model(model) {
...
// DFlash2 selector: for each draft position, score predecessor->successor
// transitions over the top-k candidates (low-rank codebooks conditioned on the
// decoder hidden) and pack [candidate ids | top_k x top_k scores] per position
// into t_h_nextn - the draft-time walk over this lattice picks the block
