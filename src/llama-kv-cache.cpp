...
ggml_tensor * llama_kv_cache::build_input_k_rot(ggml_context * ctx) const {
    ...
    // an empty cache (every layer filtered out, e.g. the unused compress stores
    // of a ratio-0-only MTP draft context) has n_embd_head_k_all == 0; the
    // doubling loop below then never terminates (0 % nrot == 0) until nrot
    // overflows to 0 -> SIGFPE
