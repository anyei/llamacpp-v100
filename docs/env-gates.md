...
| `LLAMA_SPEC_TREE` | bool (value-parsed) | off | (#132) Multi-candidate verification v1: one alt branch per verify round (drafter runner-up at draft position 0 + shared continuation) on a per-slot spare seq; rescued first-position rejections bank ~2 extra tokens. Requires `--kv-unified` (WARN + tree disabled without) + a roster of draft-simple / dspark / dflash / draft-mtp only (draft-mtp admitted by #137 branch heal; eagle3 stays excluded) + PEARL off — type-gated at load, WARN otherwise. Lossless: temp-0 byte gate holds. `SPEC_TREE:` counters every 64 armed rounds. **MEASURED NET NEGATIVE everywhere tried (#137 V4 production A/B -34.2%; #138 Qwen 2-GPU tensor 47.2/56.3 vs 71.2/86 prose/code — the every-round wide-row + seq_cp overhead swamps the rescue). Keep off in production; the arming-policy postscript is `LLAMA_SPEC_TREE_CONF`.** |
| `LLAMA_SPEC_TREE_CONF` | float 0-1 (value-parsed) | 1.0 | (#141) Confidence-gated tree arming: with `LLAMA_SPEC_TREE` on, the branch only arms when the drafter's own top-pick probability at draft position 0 is BELOW the threshold (1.0 = arm whenever an alt exists; a missing confidence capture arms unconditionally; out-of-range values fall back to 1.0). The knob WORKS (take-rate triples by conf 0.7) but every measured point stays below no-tree base (best 44.0 vs 47.4, 1-GPU mtp n2): an armed branch costs ~3 verify rows at ~9.2 ms/row against ~1 rescued token — tree-as-built is structurally unpayable at V100 verify-row prices regardless of arming policy. Budget-neutral redesign parked in #141. |
...
> enabled by default upstream since 601ad05a9 — the gate only ever mattered on
> pre-merge images, where it forced them on.
...
| `GGML_RPC_WIRE_F16` | bool | off | **(proto 4.12)** Boundary payloads in the fused pipeline ride f16 instead of f32 (SET compressed by the coordinator + expanded by the worker; FETCH response compressed by the worker + expanded by the paired recv). Per-connection: workers on proto <=4.11 silently stay f32. LOSSY but PPL-neutral (3.7669 vs 3.7895 +/- 0.21, record roster); fleet ~+15-20% decode. Engagement: one-time `rpc: compressed boundary payloads ACTIVE` INFO line (needs `-v`/`-lv` to surface). Value-parsed since 2026-08-23 (review #6 fix): `=0`/empty = off, like `GGML_RPC_TIMING`; the compose `COORD_WIRE_*=0` disable knob now works. HISTORY NOTE: before the fix it was presence-gated, so any pre-2026-08-23 "compression off" A/B leg that used `=0` actually measured on-vs-on. |
| `GGML_RPC_WIRE_Q8` | bool | off | **(proto 4.13)** q8_0 boundary payloads (~3.76x cut, 34 B per 32 values; sizes not divisible by 32 fall back to f16/f32). Takes precedence over WIRE_F16 on workers speaking minor 13 - set BOTH so each connection rides the best format it can. PPL-neutral (3.7950 vs f32 3.7895 +/- 0.21); fleet mean 4.01 t/s, tightest spread of the wire-format legs. Value-parsed since 2026-08-23 (review #6 fix): `=0`/empty = off, like `GGML_RPC_TIMING`; the compose `COORD_WIRE_*=0` disable knob now works. HISTORY NOTE: before the fix it was presence-gated, so any pre-2026-08-23 "compression off" A/B leg that used `=0` actually measured on-vs-on. |
...
| `GGML_META_PROBE_NO_WRITEBACK` | bool (PRESENCE-gated: even `=0` is on) | off | **(TASKS #9 probe)** Skip delivering the reduced value to non-root members at reduce boundaries (chain-only fused messages, no plain set) — prices the writeback leg's true wall-clock share (~47 ms/token on the record roster; the number that motivated `GGML_META_BCAST_FUSE` level 2), which the per-boundary timing drains cannot see. **OUTPUT IS GARBAGE BY DESIGN; never serve with it.** |
