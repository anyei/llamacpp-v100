...
| `GGML_META_EXPERT_DEFER` | 0/1/2 | 0 | **(TASKS #71 Expert Deferral)** Eligible star reduces sum LOCAL contributions and distribute; wire partials drain at the next multi-contributor star reduce and are ADDED there (one-reduce staleness; last reduce always exact - the logits path never defers; `META_EXPERT_DEFER` counter line under BOUNDARY_STATS, defers==injects and LOST 0 is the structural pass). **`=1` (v2, READINESS-GATED): each candidate's socket is polled at the gather (`fused_ready`, non-blocking poll(2) on the response FIFO) - a response that already arrived is consumed exactly, only actual stragglers defer; counter line shows `ready` + defer rate. `=2` (v1, defer-ALL): measurement only - fleet +27% (5.07 vs 4.00) but COHERENCE FAIL (repetition collapse - it defers ALL wire mass one layer late); never serve with it.** `EXPERT_DEFER=` in run-ep-fleet-hy3-spec.sh. |
